...
def get_geo_distribution_data(request):
    ...
    try:
        ...
        province_mapping = {
            ...
            '宁夏': '宁夏回族自治区', '广西': '广西壮族自治区', '香港': '香港特别行政区',
            '澳门': '澳门特别行政区',
            '黑龙江': '黑龙江省', '吉林': '吉林省', '辽宁': '辽宁省', '甘肃': '甘肃省',
            '青海': '青海省', '陕西': '陕西省', '山东': '山东省', '河南': '河南省',
            '江苏': '江苏省', '安徽': '安徽省', '湖北': '湖北省', '四川': '四川省',
            '浙江': '浙江省', '贵州': '贵州省', '云南': '云南省', '海南': '海南省',
            '台湾': '台湾省', '广东': '广东省', '湖南': '湖南省', '河北': '河北省',
            '山西': '山西省', '江西': '江西省', '福建': '福建省'
        }
        
        # 城市与省份的对应关系
        city_to_province = {
            '北京': '北京市', '上海': '上海市', '天津': '天津市', '重庆': '重庆市',
            '广州': '广东省', '深圳': '广东省', '东莞': '广东省', '佛山': '广东省', '惠州': '广东省', '中山': '广东省',
            '成都': '四川省', '绵阳': '四川省',
        ...
        for loc in locations:
            ...
            if province:
                ...
                # 尝试直接从城市名提取省份
                for prov_key in province_mapping.keys():
                    if city.startswith(prov_key):
                        province = province_mapping[prov_key]
                        province_counter[province] += count
                        break
        
        # 转换为地图所需的数据格式 [{'name': '省份名', 'value': 数量}, ...]
        map_data = []
        ...
        print(f"生成地理数据: {len(city_data)}个城市, {len(map_data)}个省份")
        
        if not map_data and not city_data:
    ...
    try:
        ...
        if not db:
            ...
            return JsonResponse({
                ...
                # 添加新的测试数据集
                'salary_data': {
                    'categories': ['应届毕业生', '1年以下', '1-3年', '3-5年', '5-10年', '10年以上', '不限'],
                    'data': [8, 12, 18, 25, 35, 45, 15]
                },
                'industry_data': {
                    'categories': ['互联网', '金融', '教育', '医疗', '制造业'],
                    'series': [
                        {'name': '应届毕业生', 'data': [40, 10, 30, 15, 5]},
                        {'name': '1-3年', 'data': [30, 20, 15, 20, 15]},
                        {'name': '3-5年', 'data': [20, 30, 10, 25, 20]},
                        {'name': '5-10年', 'data': [10, 35, 5, 30, 20]}
                    ]
                },
                'trend_data': {
                    'years': ['2019', '2020', '2021', '2022', '2023'],
                    'series': [
                        {'name': '应届毕业生', 'data': [10, 12, 8, 9, 11]},
                        {'name': '1-3年', 'data': [25, 28, 30, 32, 35]},
                        {'name': '3-5年', 'data': [20, 18, 22, 25, 24]},
                        {'name': '5年以上', 'data': [15, 17, 19, 22, 25]}
                    ]
                }
            })
        ...
        # 获取薪资与经验的关系数据
        # 示例聚合查询：按经验分组计算平均薪资
        # 真实环境应该从salary字段中提取数值并计算
        salary_data = {
            'categories': ['应届毕业生', '1年以下', '1-3年', '3-5年', '5-10年', '10年以上', '不限'],
            'data': [8, 12, 18, 25, 35, 45, 15]  # 这里应该是真实数据，这里用示例代替
        }
